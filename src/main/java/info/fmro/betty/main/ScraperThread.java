...
public class ScraperThread
    ...
    final long delayGetScraperEvents;
    final String threadId, cacheFileName, saveFolder;
    final AtomicLong lastTimedPageSave = new AtomicLong(); // only changed for the saves with timer, the others don't use it at all
    final AtomicLong lastGetScraperEvents = new AtomicLong(), lastPageGet = new AtomicLong(), lastUpdatedScraperEvent = new AtomicLong();
    final AtomicInteger timedScraperCounter = new AtomicInteger(), majorScrapingError = new AtomicInteger();
    final AtomicBoolean mustSavePage = new AtomicBoolean(), mustRefreshPage = new AtomicBoolean();
    ...
    @Nullable
    final AverageLogger averageLogger, averageLoggerFull;
...
//    }
    ...
    ScraperThread(final String id, final long delay, final BrowserVersion browserVersion, final int cacheMaxSize, final boolean singleLogger) {
        this.threadId = id;
